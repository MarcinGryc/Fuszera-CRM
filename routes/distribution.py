...
def distribution_export():
    contacts = DistributionContact.query.order_by(DistributionContact.list_type, DistributionContact.email).all()

    rows = [
        [
            c.id,
            "B2B" if c.list_type == "b2b" else "Indywidualni",
            c.name,
            c.email,
            c.company,
            c.notes,
    ...
    return export_xlsx(
        ...
        [("Listy", ["ID", "Lista", "Nazwa", "Email", "Firma", "Notatki"], rows)],
    )
